/**
 * This class locates resources (configuration files, images). It searches for user resources in the user directory (current
...
public final class ResourceLocator {
	...
	public static URL getURL(String resourceFileName, String defaultResourceFileName,
		...
		if (url == null) {
			url = locateAbsolute("file:/"
					+ resourceFileName.replace(System.getProperty("file.separator").charAt(0), '/'));
		}
		if (url == null) {
			// if adding leading slash was not appropriate
			url = locateAbsolute("file:/"
					+ resourceFileName.substring(1).replace(
							System.getProperty("file.separator").charAt(0), '/'));
		}
		...
		if (url != null) {
			...
			log.debug("Resource '" + resourceFileName + "' and '" + defaultResourceFileName
					+ "' not found.");
		}
	...
	/**
	 * Test if 'child' is a delegation child of 'parent'. This works only for
	 * classloaders that set their parent pointers correctly. 'null' is
	 * interpreted as the primordial loader (everybody's parent).
	 * 
	 * @param parent
	 *            the parent ClassLoader in question
	 * @param child
	 *            the child ClassLoader in question
	 * @return true if 'child' is a delegation child of 'parent' (or if
	 *         'parent'=='child')
	 */
	...
	private static URL locate(String resourceFileName, ClassLoader loader, Class<?> caller) {
		...
		// try using caller class (can load from paths relative to caller
		// package)
	 ...
	 * @return URL of the file or null if not found
	 */
	private static URL locateDefault(String defaultResourceFileName, String resourceFileName,
			Class<?> caller) {
		...
		String codesourcelocation = caller.getProtectionDomain().getCodeSource().getLocation()
				.toString();
		if (codesourcelocation.endsWith("jar")) {
			// try beside or in jar containing the caller class
			urlstring = codesourcelocation.substring(0, codesourcelocation.lastIndexOf("/"))
					+ resourceFileName;
