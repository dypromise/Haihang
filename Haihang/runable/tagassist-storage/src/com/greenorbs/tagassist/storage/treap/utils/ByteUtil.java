...
import com.greenorbs.tagassist.storage.treap.disk.DiskTreapHeader;
import com.greenorbs.tagassist.storage.treap.disk.DiskTreapNode;
...
public class ByteUtil {
	...
	public static byte[] dumpV_MS(Comparable key, Serializable value) throws Exception {
		...
		if(key instanceof FastString){
			FastString fkey = (FastString)key;
			key_length = (byte)fkey.bytes.length;
			key_bytes = fkey.bytes;
		}
		else{
			key_bytes = key.toString().getBytes();
			key_length =(byte) key_bytes.length ;
		}
	...
	public static ByteBuffer dumps(DiskTreapNode node,int maxBlockSize){
		...
		if(node.key instanceof FastString){
			...
			if(bytes.length+DiskTreapNode.STRING_KEY_OVER_HEAD>maxBlockSize){
				throw new RuntimeException("key is too long(键太长了,可以TreapDB启动时调大索引块的size,但最大只能为127个字节)");
			}
			buf.put((byte)13);
