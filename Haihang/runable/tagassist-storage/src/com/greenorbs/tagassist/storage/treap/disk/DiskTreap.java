...
import com.greenorbs.tagassist.storage.treap.utils.ByteUtil;
import com.greenorbs.tagassist.storage.treap.utils.FastString;

public class DiskTreap<K extends Comparable<K>, V extends Serializable>
		implements ITreap<K, V> {
	...
	public DiskTreap(int block_size, File _indexFile, long mmapSize)
			throws Exception {
	...
	public void optimize(int swapCount) {
		...
		try {
			...
			while (!queue.isEmpty()) {
				...
				DiskTreapNode<K, V> tmpNode = this.blockUtil.readNode(
						nodeNumber, false);
				setA.add(tmpNode);
	...
	private void collectKMax(int startNode, int k, Map<K, V> result)
			throws Exception {
		if (startNode == -1) {
			return;
		}
	...
	private void collectKMin(int startNode, int k, Map<K, V> result)
			throws Exception {
		if (startNode == -1) {
			return;
		}
	...
	private int remove(int startNode, K key) throws Exception {
		if (startNode == -1)
			return -1;
		DiskTreapNode<K, V> currentNode = this.blockUtil.readNode(startNode,
				false);
		...
		if (cp == 0) {
			...
			if (currentNode.lNo == -1 && currentNode.rNo == -1) {
				...
				DiskTreapNode<K, V> leftNode = this.blockUtil.readNode(
						currentNode.lNo, false);
				DiskTreapNode<K, V> rightNode = this.blockUtil.readNode(
						currentNode.rNo, false);
				if (leftNode.fix < rightNode.fix) {
					int newstartNodeNo = rotateRight(startNode);
					DiskTreapNode<K, V> newstartNode = this.blockUtil.readNode(
							newstartNodeNo, false);
					newstartNode.rNo = remove(startNode, key);
					if (newstartNode.rNo == -1) {
						newstartNode.r_size = 0;
					} else {
						DiskTreapNode<K, V> rN = this.blockUtil.readNode(
								newstartNode.rNo, false);
						newstartNode.r_size = 1 + rN.l_size + rN.r_size;
					}
					this.blockUtil.writeNode(newstartNodeNo, newstartNode,
							false);
					...
					if (newstartNode.lNo == -1) {
						...
						DiskTreapNode<K, V> lN = this.blockUtil.readNode(
								newstartNode.lNo, false);
	...
	public Map<K, V> prefix(K prefixString, int limit, K startK, boolean asc) {
		...
		try {
			...
			DiskTreapHeader header = this.blockUtil.readHeader();
			prefixSearch(header.rootNo, prefixString, results, limit, startK,
					asc);
			return results;
		} catch (Exception e) {
			throw new RuntimeException(e);
	...
	private void prefixSearch(int startNode, K prefixString, Map<K, V> results,
			int limit, K startK, boolean asc) throws Exception {
		if (startNode == -1)
			return;
		...
		if (prefixString.compareTo(cur.key) <= 0) {
			if (isPrefixString(prefixString.toString(), cur.key.toString())) {
				if (asc) {
					prefixSearch(cur.lNo, prefixString, results, limit, startK,
							asc);
					if (results.size() >= limit)
						return;
					this.blockUtil.filleNodeValue(cur);
					results.put(cur.key, cur.value);
					prefixSearch(cur.rNo, prefixString, results, limit, startK,
							asc);
				} else {
					prefixSearch(cur.rNo, prefixString, results, limit, startK,
							asc);
	...
	private boolean isPrefixString(String prefixString, String key) {
		if (key.indexOf(prefixString) == 0)
			return true;
		return false;
	}

	private void pushDeletedNode(int nodeNo, DiskTreapNode<K, V> deletedNode)
			throws Exception {
		DiskTreapHeader header = this.blockUtil.readHeader();
	...
	private int popDeletedNode() throws Exception {
		...
		if (header.deletedNode == -1) {
			...
			DiskTreapNode<K, V> topNode = this.blockUtil.readNode(topNodeNo,
					false);
	...
	private int insert(int startNode, K key, V value) throws Exception {
		if (startNode == -1) {
			...
			DiskTreapNode<K, V> currentNode = this.blockUtil.readNode(
					startNode, false);
			int cp = currentNode.key.compareTo(key);
			if (cp == 0) {
	...
	private int findWithParent(int startNode, K key,
			Map<Integer, Integer> parentMap, Map<Integer, Integer> directionMap)
			throws Exception {
		if (startNode == -1) {
			return -1;
		} else {
			...
			if (cp == 0) {
				...
				return findWithParent(currentNode.rNo, key, parentMap,
						directionMap);
			} else if (cp > 0) {
				parentMap.put(currentNode.lNo, startNode);
				directionMap.put(currentNode.lNo, 0); // left child
				return findWithParent(currentNode.lNo, key, parentMap,
						directionMap);
			}
		}
		return -1;
	...
	private void prevSearch(int startNode, K key, Map<K, V> results, int limit)
			throws Exception {
		if (startNode == -1)
			return;
		if (results.size() >= limit)
	...
	private void nextSearch(int startNode, K key, Map<K, V> results, int limit)
			throws Exception {
		if (startNode == -1)
			return;
		if (results.size() >= limit)
	...
	private void realBulkGet(int startNode, NavigableSet<K> keySet,
			Map<K, V> result) throws Exception {
		if (startNode == -1 || keySet.isEmpty())
			return;
		DiskTreapNode<K, V> cur = this.blockUtil.readNode(startNode, false);
	...
	private int bulkInsert(int startNode, NavigableMap<K, V> pairs)
			throws Exception {
		if (pairs.isEmpty())
			return startNode;
		if (startNode == -1) {
			...
			NavigableMap<K, V> rightPart = pairs
					.tailMap(currentNode.key, false);
	...
	public Map<K, V> bulkPrefix(List<String> prefixList, int limit, K startK,
		...
		try {
			...
			realBulkPrefix(header.rootNo, keySet, result, limit, startK, asc,
					counter);
			return result;
		} catch (Exception e) {
			throw new RuntimeException(e);
	...
	private void realBulkPrefix(int startNode, TreeSet<String> keySet,
			Map<K, V> result, int limit, K startK, boolean asc,
			Map<String, Integer> counter) throws Exception {
		if (startNode == -1 || keySet.isEmpty())
			return;
		...
		if (asc) {// 顺序
			if (startK != null && startK.compareTo(cur.key) > 0) {
				realBulkPrefix(cur.rNo, keySet, result, limit, startK, asc,
						counter);
				return;
			}
			...
			if (startK != null && startK.compareTo(cur.key) < 0) {
				realBulkPrefix(cur.lNo, keySet, result, limit, startK, asc,
						counter);
				return;
			}
			realBulkPrefix(cur.rNo, right, result, limit, startK, asc, counter);
	...
	private int realRemovePrefix(int startNode, K prefixString)
			throws Exception {
		if (startNode == -1)
			return -1;
		DiskTreapNode<K, V> currentNode = this.blockUtil.readNode(startNode,
		...
		if (cp <= 0) {
			if (isPrefixString(prefixString.toString(),
					currentNode.key.toString())) {
				if (currentNode.fix != Integer.MAX_VALUE) {
					// 在data文件中加入删除记录信息
					currentNode.fix = Integer.MAX_VALUE;
					this.blockUtil.writeNode(startNode, currentNode, false);
				...
				if (currentNode.lNo == -1 && currentNode.rNo == -1) {
					...
					newstartNode.rNo = realRemovePrefix(newstartNode.rNo,
							prefixString);
					if (newstartNode.rNo == -1) {
						newstartNode.r_size = 0;
					} else {
					...
					newstartNode.lNo = realRemovePrefix(newstartNode.lNo,
							prefixString);
					if (newstartNode.lNo == -1) {
						newstartNode.l_size = 0;
					} else {
					...
					this.blockUtil.writeNode(newstartNodeNo, newstartNode,
							false);
					if (isPrefixString(prefixString.toString(),
							newstartNode.key.toString())) {
						return realRemovePrefix(newstartNodeNo, prefixString);
					}
					return newstartNodeNo;
				}
			} else {
				currentNode.lNo = realRemovePrefix(currentNode.lNo,
						prefixString);
				if (currentNode.lNo == -1) {
					currentNode.l_size = 0;
				} else {
	 ...
	 * @throws Exception
	 */
	private Entry<K, V> findKth(int rootNo, int k, boolean asc)
			throws Exception {
		if (rootNo == -1) {
			return null;
		}
		final DiskTreapNode<K, V> startNode = this.blockUtil.readNode(rootNo,
				false);
		if (asc) {
...
//	
//		DiskTreap<FastString, BaggageInfo> treap = new DiskTreap<FastString, BaggageInfo>(
//				64, new File("temp\\storage"), 128);
////		for(long i=0;i<1000000;i++){
////			BaggageInfo b = new BaggageInfo();
////			b.setNumber(String.valueOf(i));
////			treap.put(new FastString("a"+i), b);
////		}
//		
//		System.out.println(treap.get(new FastString("a"+130)).getNumber());
//	}
}
