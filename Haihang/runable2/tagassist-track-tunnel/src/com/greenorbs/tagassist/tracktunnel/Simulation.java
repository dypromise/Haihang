...
public class Simulation {
	 ...
	 */
	public static void main(String[] args) throws MessageBusException, ParseException{
...
//		
		...
		while(true){
			...
			VFlightUpdated vFlightUpdated = new VFlightUpdated();
			
			//BaggageTracked baggageTracked = new BaggageTracked();
			
			int flight_choice = random.nextInt(2);
			int track_choice = random.nextInt(3);
			...
			/*if(number_int < 10)
				...
				*/
			String flightId = FlightUtils.parseFlightCode(flightid[flight_choice]);
			try {
				//丁杨改；
				vBaggageTracked.setEPC(EPC96Utils.hexEncode(number_str, flightId));
				vBaggageTracked.setFlightId(flightId);
				vBaggageTracked.setDistance(track_distance[track_choice]);
				
				vFlightUpdated.setFlightId(flightId);
				
				//vFlightUpdated.setMissing(missing);
				/*baggageTracked.setEPC(EPC96Utils.hexEncode(number_str, string));
				baggageTracked.setDistance(track_distance[track_choice]);
				baggageTracked.setSource(track_uuid[track_choice]);
				*/
				//需要显示下这个publish信息！
				//丁杨 改.
				_logger.info("Message published: " + vFlightUpdated);
				publisher.publish(vFlightUpdated);
				Thread.sleep(random.nextInt(1000)+100);
				
				_logger.info("Message published: " + vBaggageTracked);
				publisher.publish(vBaggageTracked);
				//Thread.sleep(random.nextInt(1000)+100);
