...
public class BeiyangTrackReader2 extends AbstractBeiyangReader implements
		IReader {
	...
	public BeiyangTrackReader2(String ip, int port) {
	...
	private void openReader() throws HardwareException {
		...
		if (this._handle < 0) {
			_log.error("It fails to open the Beiyang reader. The error code is: "
					+ this._handle);

			throw new HardwareException(
					"It fails to open the Beiyang reader.");
		}

		_log.info("It is successful to open the Beiyang reader, port=" + this._port
				+ ", ip=" + this._ip);
	...
	private void closeReader() throws HardwareException {
		...
		if (rtn != 0) {
			_log.error("It fails to close the Beiyang reader. The error code is: "
					+ String.valueOf(rtn));
			throw new HardwareException("It fails to close the Beiyang Reader.");
		}

		_log.info("It is successful to close the Beiyang Reader.");
	...
	public void startup() throws HardwareException {
		...
		if (this._status == IHardware.STATUS_ON) {
			_log.info("The reader is already on.");
			return;
		}
	...
	public int getReadPointPower(String readPoint) throws HardwareException {
		...
		try {
			...
			e.printStackTrace();
			throw new HardwareException(
					"There is an error when getting the read point power.");
		}
	...
	public boolean setReadPointPower(String readpoint, int value)
		...
		try {
			...
			throw new HardwareException(
					"There is an error when setting the read point power.");
		}
	...
	public ObservationReport identify(String[] readPoints)
		...
		try {
			...
			while (true) {
				...
				if (result != 0 && result != 100) {
					_log.error("It fails to getInventTag6CWithRSSI. The result code is "
							+ result);
					throw new HardwareException();
				}
	...
	public void run() {
		...
		while (this._status == IHardware.STATUS_ON) {
			if (this._identifyListeners.size() > 0) {
				try {
					ObservationReport report = this.identify(this.getReadPoints());
